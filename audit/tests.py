"""The audit trail is written from Sprint 1 on, and tampering with it shows."""
...
class AuditTrailTests(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create_user("weaver@tuchin.co", "Vueltiao-2026")
...
class ChainTests(TestCase):
    ...
    The guard on `save()` is Python. Raw SQL never calls it, so these are the
    cases that matter: the chain is what makes them visible.
    """
    ...
    def test_rewriting_an_entry_with_raw_sql_is_detected(self):
        target = AuditEntry.objects.order_by("id").first()
